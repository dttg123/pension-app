...
(()=>{
'use strict';
const V1_APP_VERSION='2.0.0';
const V1_SCHEMA_VERSION=6;
const V1_BACKUP_FORMAT='1.1';
const V1_PROJECT_BACKUP_FORMAT='1.0';
const V1_PROJECT_ASSETS=['index.html','base.css','components.css','features.css','core.js','ui.js','analysis.js','ocr.js','backup.js','planning.js','ledger.js','coach.js','integrity.js','manifest.webmanifest','sw.js'];
...
window.PensionV1Record={
  ...
  snapshot(reason='asset-update',effectiveDate=''){
    state.snapshots.push({id:v1Uid('snapshot'),createdAt:v1DateTime(),effectiveDate:effectiveDate||state.lastUpdated||'',reason,accounts:clone(state.accounts)});
    v1AddLedger('snapshot',{date:v1DateTime(),note:`자산현황 갱신 · ${effectiveDate||state.lastUpdated||''}`,source:'asset-snapshot'});
  }
};
...
`;
  ...
  if(!verified.ok)throw new Error(verified.errors[0]);
  return {bytes:zip,blob:new Blob([zip],{type:'application/zip'}),manifest,verification:verified,dataPack,fileName:`PENSION_PROJECT_BACKUP_${v1FileStamp()}.zip`};
}
...
async function v1DriveBackup(){try{v1ShowResult('프로젝트 백업 준비 중','현재 앱 15개 파일과 검증된 데이터 ZIP을 묶고 있습니다.');await v1PersistNow();const pack=await v1BuildProjectBackup(),drive=v1Adapters().drive;if(drive?.upload){const result=await drive.upload(new File([pack.blob],pack.fileName,{type:'application/zip'}),{folder:'투자시스템/pension/project-backups',backupType:'project',manifest:pack.manifest});if(drive.download&&result?.fileId){const remote=await drive.download(result.fileId),hashA=await v1Sha(pack.bytes),hashB=await v1Sha(new Uint8Array(await remote.arrayBuffer()));if(hashA!==hashB)throw new Error('Drive 재다운로드 체크섬이 일치하지 않습니다.')}state.meta.storage.driveStatus='정상';state.meta.storage.lastDriveBackup=v1LocalTime();save();v1ShowResult('Drive 프로젝트 백업 검증 완료',`${esc(pack.fileName)}<br>앱 ${pack.manifest.appAssetCount}개 · 데이터 ZIP ${v1FormatSize(pack.dataPack.bytes.length)} · 업로드 체크섬 일치`);return}const file=new File([pack.blob],pack.fileName,{type:'application/zip'});if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({title:'개인연금 프로젝트 전체 백업',text:'Google Drive를 선택해 투자시스템/개인연금/프로젝트백업 폴더에 저장하세요.',files:[file]});state.meta.storage.driveStatus='공유 완료';state.meta.storage.lastDriveBackup=v1LocalTime();save();v1ShowResult('Google Drive 선택창 전달 완료',`검증된 ${esc(pack.fileName)}을 전달했습니다.<br>앱 15개와 현재 데이터가 함께 들어 있습니다.`)}else{v1Download(pack.blob,pack.fileName);v1ShowResult('프로젝트 ZIP 다운로드 완료',`직접 Drive 연결이나 안드로이드 공유가 없어 폰으로 내려받았습니다.<br>Google Drive의 투자시스템/개인연금/프로젝트백업 폴더에 올려주세요.`)}}catch(e){v1ShowResult('프로젝트 백업 중단',esc(e?.message||e),true)}}
