/* 개인연금 V3.1.1 데이터·백업 엔진 */

/* ===== js/50-data-engine.js ===== */
(()=>{
'use strict';
const V1_APP_VERSION='3.1.1';
const V1_SCHEMA_VERSION=6;
const V1_BACKUP_FORMAT='1.1';
const V1_PROJECT_BACKUP_FORMAT='1.0';
const V1_PROJECT_ASSETS=['index.html','base.css','components.css','features.css','v21.css','core.js','ui.js','analysis.js','ocr.js','backup.js','planning.js','ledger.js','coach.js','integrity.js','charts.js','v21.js','v29.css','v29.js','manifest.webmanifest','sw.js','icon.svg'];
...
let v1DBPromise=null,v1PersistTimer=null,v1PersistChain=Promise.resolve(),v1RestoreCandidate=null,v1LastCore='';
...
function v1EnsureState(target=state){
  ...
  if(Number(target.schemaVersion)>V1_SCHEMA_VERSION){target.meta=target.meta||{};target.meta.storage={localStatus:'읽기 전용',localVerification:'미검증',cloudStatus:'중단',driveStatus:'중단',driveVerification:'미검증',lastLocalSave:'',lastCloudSave:'',lastDriveBackup:'',lastDriveShare:'',lastBackup:'',lastRestore:'',lastValidation:'',...(target.meta.storage||{})};return target;}
  ...
  target.meta.storage={localStatus:'대기',localVerification:'미검증',cloudStatus:'연결 전',driveStatus:'연결 전',driveVerification:'미검증',lastLocalSave:'',lastCloudSave:'',lastDriveBackup:'',lastDriveShare:'',lastBackup:'',lastRestore:'',lastValidation:'',...(target.meta.storage||{})};
  return target;
}
...
window.PensionV1Record={
  ...
  snapshot(reason='asset-update',effectiveDate=''){
    /* V2.9: 종목별 상세 스냅샷은 누적하지 않는다. 최신 보유자산은 현재 상태로 유지하고,
...
async function v1PersistOnce({strict=false,syncCloud=true}={}){
  v1EnsureState();
  if(state.meta?.compatibilityReadOnly)throw new Error('더 새로운 데이터 구조라 저장할 수 없습니다.');
  const storage=state.meta.storage;
  storage.localStatus='저장 중';storage.localVerification='확인 중';delete storage.localError;v1RefreshDataCenter();
  try{
    const stamp=v1LocalTime();storage.localStatus='정상';storage.localVerification='읽기검증 완료';storage.lastLocalSave=stamp;
    const copy=clone(state);await v1DBPut(V1_CURRENT,copy);
    const readback=await v1DBGet(V1_CURRENT);
    if(!readback||v1CoreSignature(readback)!==v1CoreSignature(copy)||readback.meta?.storage?.lastLocalSave!==stamp||readback.meta?.storage?.localStatus!=='정상')throw new Error('기기 저장 후 읽기검증이 일치하지 않습니다.');
    try{localStorage.setItem('pension-v1',JSON.stringify(copy));const mirror=JSON.parse(localStorage.getItem('pension-v1')||'null');if(!mirror||v1CoreSignature(mirror)!==v1CoreSignature(copy))throw new Error('브라우저 보조 저장소 읽기검증이 일치하지 않습니다.')}catch(e){storage.localMirrorError=String(e?.message||e)}
  }catch(e){storage.localStatus='실패';storage.localVerification='실패';storage.localError=String(e?.message||e);v1RefreshDataCenter();if(strict)throw e;return {ok:false,local:{ok:false,error:storage.localError},cloud:{ok:false,skipped:true}}}
  const result={ok:true,local:{ok:true,verified:true},cloud:{ok:false,skipped:true}};
  const cloud=v1Adapters().cloud;
  if(syncCloud&&cloud?.save){
    storage.cloudStatus='동기화 중';delete storage.cloudError;v1RefreshDataCenter();
    try{const remoteResult=await cloud.save(clone(state));storage.cloudStatus='정상';storage.lastCloudSave=v1LocalTime();storage.cloudRevision=Number(remoteResult?.revision??state.meta.revision??0);result.cloud={ok:true,revision:storage.cloudRevision}}
    catch(e){storage.cloudStatus='실패';storage.cloudError=String(e?.message||e);result.cloud={ok:false,error:storage.cloudError}}
  }else if(!cloud?.save)storage.cloudStatus='연결 전';
  try{
    const finalCopy=clone(state);await v1DBPut(V1_CURRENT,finalCopy);const finalRead=await v1DBGet(V1_CURRENT);
    if(!finalRead||v1CoreSignature(finalRead)!==v1CoreSignature(finalCopy)||finalRead.meta?.storage?.localStatus!=='정상')throw new Error('최종 저장 상태 읽기검증이 일치하지 않습니다.');
    try{localStorage.setItem('pension-v1',JSON.stringify(finalCopy))}catch(_){ }
  }catch(e){storage.localStatus='실패';storage.localVerification='실패';storage.localError=String(e?.message||e);result.ok=false;result.local={ok:false,error:storage.localError};if(strict)throw e}
  v1RefreshDataCenter();return result;
}
function v1PersistNow(options={}){
  const run=()=>v1PersistOnce(options),pending=v1PersistChain.then(run,run);v1PersistChain=pending.catch(()=>{});return pending;
}
function v1QueuePersist(){clearTimeout(v1PersistTimer);v1PersistTimer=setTimeout(()=>{v1PersistNow().catch(()=>{})},120)}
...
function v1ParseCsv(text){
  const src=String(text||'').replace(/^\uFEFF/,'');const rows=[];let row=[],cell='',quoted=false;
  for(let i=0;i<src.length;i++){const ch=src[i];if(quoted){if(ch==='"'&&src[i+1]==='"'){cell+='"';i++}else if(ch==='"')quoted=false;else cell+=ch}else if(ch==='"')quoted=true;else if(ch===','){row.push(cell);cell=''}else if(ch==='\n'){row.push(cell);rows.push(row);row=[];cell=''}else if(ch!=='\r')cell+=ch}
  if(quoted)throw new Error('CSV 따옴표가 닫히지 않았습니다.');if(cell!==''||row.length){row.push(cell);rows.push(row)}return rows;
}
const V1_CSV_HEADERS={
  'accounts.csv':['rowType','accountId','accountName','holdingId','assetName','assetClass','quantity','cost','marketValue','cash','principal'],
  'transactions.csv':['id','type','date','accountId','assetId','assetName','amount','quantity','note','source'],
  'contributions.csv':['id','type','date','accountId','assetId','assetName','amount','quantity','note','source'],
  'dividends.csv':['id','type','date','accountId','assetId','assetName','amount','quantity','note','source'],
  'snapshots.csv':['id','createdAt','effectiveDate','reason','accountCount','totalAssets'],
  'archived-holdings.csv':['archiveId','entityId','accountId','archivedAt','reason','assetName','quantity','cost','marketValue'],
  'settings.csv':['key','value']
};
function v1CsvObjects(bytes,name){const rows=v1ParseCsv(v1Dec.decode(bytes));const expected=V1_CSV_HEADERS[name];if(!rows.length)throw new Error(`CSV가 비어 있습니다: ${name}`);if(rows[0].join('\u001f')!==expected.join('\u001f'))throw new Error(`CSV 머리글이 표준과 다릅니다: ${name}`);return rows.slice(1).filter(r=>r.some(v=>v!=='' )).map((r,i)=>{if(r.length!==expected.length)throw new Error(`CSV 열 개수가 다릅니다: ${name} ${i+2}행`);return Object.fromEntries(expected.map((h,j)=>[h,r[j]]))})}
function v1SameIdSet(rows,key,expected,label){const ids=rows.map(r=>r[key]).filter(Boolean),unique=new Set(ids);if(unique.size!==ids.length)throw new Error(`${label} CSV ID가 중복되었습니다.`);const wanted=new Set(expected.map(String));if(ids.length!==wanted.size||ids.some(id=>!wanted.has(String(id))))throw new Error(`${label} CSV 행 또는 ID가 원본과 다릅니다.`)}
function v1ValidateCsvFiles(files,data){
  const parsed={};for(const name of Object.keys(V1_CSV_HEADERS))parsed[name]=v1CsvObjects(files[name],name);
  const counts=v1Counts(data),accounts=parsed['accounts.csv'],accountRows=accounts.filter(r=>r.rowType==='account'),holdingRows=accounts.filter(r=>r.rowType==='holding');if(accounts.some(r=>!['account','holding'].includes(r.rowType)))throw new Error('accounts.csv의 rowType 값이 올바르지 않습니다.');
  v1SameIdSet(accountRows,'accountId',Object.values(data.accounts||{}).map(a=>a.id),'계좌');v1SameIdSet(holdingRows,'holdingId',Object.values(data.accounts||{}).flatMap(a=>(a.holdings||[]).map(h=>h.id)),'보유상품');if(accountRows.length!==counts.accounts||holdingRows.length!==counts.holdings)throw new Error('accounts.csv 행 수가 원본과 다릅니다.');
  const ledger=data.ledger||[],contribTypes=new Set(['contribution','contribution-adjustment']),dividendTypes=new Set(['dividend','dividend-adjustment','dividend-account-allocation']);
  v1SameIdSet(parsed['transactions.csv'],'id',ledger.map(r=>r.id),'전체 원장');v1SameIdSet(parsed['contributions.csv'],'id',ledger.filter(r=>contribTypes.has(r.type)).map(r=>r.id),'납입');v1SameIdSet(parsed['dividends.csv'],'id',ledger.filter(r=>dividendTypes.has(r.type)).map(r=>r.id),'배당');v1SameIdSet(parsed['snapshots.csv'],'id',(data.snapshots||[]).map(r=>r.id),'스냅샷');v1SameIdSet(parsed['archived-holdings.csv'],'archiveId',(data.archives?.holdings||[]).map(r=>r.archiveId),'보관 보유상품');
  const settings=parsed['settings.csv'],keys=new Set(settings.map(r=>r.key));for(const key of ['profile.birthYear','profile.retirementAge','settings.monthly.pension','settings.monthly.irp'])if(!keys.has(key))throw new Error(`settings.csv 필수 설정 누락: ${key}`);
  return {ok:true,counts:{accounts:accountRows.length,holdings:holdingRows.length,transactions:parsed['transactions.csv'].length,contributions:parsed['contributions.csv'].length,dividends:parsed['dividends.csv'].length,snapshots:parsed['snapshots.csv'].length,archivedHoldings:parsed['archived-holdings.csv'].length,settings:settings.length}};
}
...
async function v1ZipRead(buffer){
  const bytes=buffer instanceof Uint8Array?buffer:new Uint8Array(buffer),MAX_FILES=200,MAX_FILE=50*1024*1024,MAX_TOTAL=100*1024*1024;
  ...
  let eocd=-1;const start=Math.max(0,bytes.length-66000);for(let i=bytes.length-22;i>=start;i--){if(can(i,4)&&v1R32(view,i)===0x06054b50){eocd=i;break}}
  if(eocd<0||!can(eocd,22))throw new Error('ZIP 끝 정보를 찾지 못했습니다. 파일이 손상되었을 수 있습니다.');
  const count=v1R16(view,eocd+10),centralSize=v1R32(view,eocd+12),centralOffset=v1R32(view,eocd+16);if(count<1||count>MAX_FILES)throw new Error('ZIP 내부 파일 개수가 비정상입니다.');if(!can(centralOffset,centralSize)||centralOffset+centralSize>eocd)throw new Error('ZIP 파일 목록 위치가 손상되었습니다.');
  const files={};let p=centralOffset,totalSize=0;
  for(let i=0;i<count;i++){
    if(!can(p,46)||v1R32(view,p)!==0x02014b50)throw new Error('ZIP 파일 목록이 손상되었습니다.');
    const flags=v1R16(view,p+8),method=v1R16(view,p+10),crcExpected=v1R32(view,p+16),compressedSize=v1R32(view,p+20),size=v1R32(view,p+24),nameLen=v1R16(view,p+28),extraLen=v1R16(view,p+30),commentLen=v1R16(view,p+32),localOffset=v1R32(view,p+42),next=p+46+nameLen+extraLen+commentLen;
    if(flags&1)throw new Error('암호화된 ZIP은 복원할 수 없습니다.');if(size>MAX_FILE)throw new Error('ZIP 내부 단일 파일이 50MB를 초과합니다.');totalSize+=size;if(totalSize>MAX_TOTAL)throw new Error('ZIP 압축 해제 총크기가 100MB를 초과합니다.');
    if(!can(p+46,nameLen)||next>centralOffset+centralSize)throw new Error('ZIP 파일 이름 또는 목록 길이가 손상되었습니다.');const name=v1Dec.decode(bytes.slice(p+46,p+46+nameLen));
    if(!name||name.length>255||name.includes('\\')||name.startsWith('/')||name.split('/').includes('..')||Object.prototype.hasOwnProperty.call(files,name))throw new Error('ZIP 내부 파일 이름이 없거나 안전하지 않거나 중복되었습니다.');
    if(!can(localOffset,30)||v1R32(view,localOffset)!==0x04034b50)throw new Error('ZIP 내부 파일 위치가 손상되었습니다.');const localFlags=v1R16(view,localOffset+6);if(localFlags&1)throw new Error('암호화된 ZIP은 복원할 수 없습니다.');const ln=v1R16(view,localOffset+26),le=v1R16(view,localOffset+28),dataStart=localOffset+30+ln+le;if(!can(dataStart,compressedSize))throw new Error(`ZIP 내부 파일이 잘렸습니다: ${name}`);
    const packed=bytes.slice(dataStart,dataStart+compressedSize);let out;if(method===0)out=packed;else if(method===8)out=await v1InflateRaw(packed);else throw new Error(`지원하지 않는 ZIP 압축 방식입니다: ${method}`);if(out.length!==size)throw new Error(`ZIP 내부 파일 크기가 일치하지 않습니다: ${name}`);if(v1Crc32(out)!==crcExpected)throw new Error(`ZIP 내부 CRC가 일치하지 않습니다: ${name}`);files[name]=out;p=next;
  }
  return files;
}
...
function v1ValidateData(data){
  ...
  if(!Number.isFinite(age)||age<18||age>100)add('현재 나이 값이 올바르지 않습니다.');
  if(!Number.isFinite(ret)||ret<18||ret>90)add('연금 개시 나이 값이 올바르지 않습니다.');
...
async function v1VerifyBackup(input){
  const errors=[];try{
    ...
    const manifest=JSON.parse(v1Dec.decode(files['manifest.json']));if(manifest.appId!=='asset-os-pension')errors.push('다른 앱의 백업입니다.');if(Number(manifest.schemaVersion)>V1_SCHEMA_VERSION)errors.push(`더 새로운 데이터 구조입니다: ${manifest.schemaVersion}`);if(String(manifest.backupFormatVersion||'')!==V1_BACKUP_FORMAT)errors.push('지원하지 않는 백업 형식입니다.');
    const requiredSet=new Set(V1_REQUIRED.filter(x=>x!=='manifest.json'));for(const name of requiredSet)if(!manifest.files?.[name])errors.push(`manifest 체크섬 정보 누락: ${name}`);for(const name of Object.keys(manifest.files||{}))if(!requiredSet.has(name))errors.push(`manifest에 알 수 없는 파일이 있습니다: ${name}`);
    ...
    let csvValidation=null;try{csvValidation=v1ValidateCsvFiles(files,data)}catch(e){errors.push(String(e?.message||e))}
    ...
    return {ok:!errors.length,errors:[...new Set(errors)],files,manifest,data,csvValidation};
...
async function v1VerifyProjectBackup(input){
  ...
  try{
    ...
    const listed=Array.isArray(manifest.appAssets)?manifest.appAssets:[],listedNames=listed.map(x=>x?.name),listedPaths=listed.map(x=>x?.path);if(new Set(listedNames).size!==listedNames.length||new Set(listedPaths).size!==listedPaths.length)errors.push('프로젝트 앱 파일 정보가 중복되었습니다.');
    for(const name of V1_PROJECT_ASSETS){
      const path=`app/${name}`,info=listed.find(x=>x.name===name);if(info&&info.path!==path)errors.push(`앱 파일 경로 정보 불일치: ${name}`);
    ...
    if(!files['RECOVERY.md']||!files['RECOVERY.md'].length)errors.push('프로젝트 복구 안내서가 없습니다.');if(dataVerification?.ok){for(const [key,value] of Object.entries(manifest.dataBackup?.counts||{}))if(Number(value)!==Number(dataVerification.manifest.counts?.[key]))errors.push(`프로젝트 데이터 건수 불일치: ${key}`);}
...
async function v1PhoneBackup(){try{v1ShowResult('데이터 백업 생성 중','기기 저장 읽기검증 후 원본·CSV·ZIP·체크섬을 다시 검사합니다.');await v1PersistNow({strict:true,syncCloud:false});const pack=await v1BuildBackup(),saved=await v1SaveBlobToPhone(pack.blob,pack.fileName);state.meta.storage.lastBackup=v1LocalTime();state.meta.storage.lastValidation='데이터 백업 정상 · CSV 건수 검증';await v1PersistNow({strict:true,syncCloud:false});v1ShowResult(saved.method==='picker'?'폰 데이터 ZIP 저장 완료':'폰 데이터 ZIP 다운로드 요청 완료',`파일명 ${esc(saved.name)}<br>${saved.method==='picker'?'선택한 폴더에 저장했습니다.':'브라우저 기본 다운로드 폴더에 저장됩니다.'}<br>크기 ${v1FormatSize(pack.bytes.length)} · 계좌 ${pack.manifest.counts.accounts}개 · 보유상품 ${pack.manifest.counts.holdings}개 · 기록 ${pack.manifest.counts.transactions}건<br>JSON·CSV 건수·체크섬·총자산 ${fmt(pack.manifest.counts.totalAssets)} 일치`)}catch(e){v1ShowResult('데이터 백업 생성 중단',esc(e?.message||e),true)}}
async function v1DriveBackup(){try{v1ShowResult('프로젝트 백업 준비 중',`현재 앱 ${V1_PROJECT_ASSETS.length}개 파일과 검증된 데이터 ZIP을 묶고 있습니다.`);await v1PersistNow({strict:true,syncCloud:false});const pack=await v1BuildProjectBackup(),drive=v1Adapters().drive;if(drive?.upload){const result=await drive.upload(new File([pack.blob],pack.fileName,{type:'application/zip'}),{folder:'투자시스템/pension/project-backups',backupType:'project',manifest:pack.manifest});if(!result?.fileId)throw new Error('Drive 업로드 응답에 파일 ID가 없어 완료를 확인할 수 없습니다.');if(Number.isFinite(Number(result.size))&&Number(result.size)!==pack.bytes.length)throw new Error('Drive 업로드 응답 파일 크기가 일치하지 않습니다.');state.meta.storage.driveStatus='업로드 완료·미검증';state.meta.storage.driveVerification='미검증';state.meta.storage.lastDriveUpload=v1LocalTime();if(drive.download){const remote=await drive.download(result.fileId),remoteBytes=new Uint8Array(await remote.arrayBuffer()),hashA=await v1Sha(pack.bytes),hashB=await v1Sha(remoteBytes);if(remoteBytes.length!==pack.bytes.length||hashA!==hashB)throw new Error('Drive 재다운로드 크기 또는 체크섬이 일치하지 않습니다.');state.meta.storage.driveStatus='정상';state.meta.storage.driveVerification='sha256';state.meta.storage.lastDriveBackup=v1LocalTime();await v1PersistNow({strict:true,syncCloud:false});v1ShowResult('Drive 프로젝트 백업 검증 완료',`${esc(pack.fileName)}<br>앱 ${pack.manifest.appAssetCount}개 · 데이터 ZIP ${v1FormatSize(pack.dataPack.bytes.length)} · 재다운로드 SHA-256 일치`)}else{await v1PersistNow({strict:true,syncCloud:false});v1ShowResult('Drive 업로드 완료 · 재검증 필요',`${esc(pack.fileName)}<br>파일 ID와 크기는 확인했지만 Drive 재다운로드 기능이 없어 SHA-256 검증은 하지 못했습니다.`)}return}const file=new File([pack.blob],pack.fileName,{type:'application/zip'});if(navigator.share&&(!navigator.canShare||navigator.canShare({files:[file]}))){await navigator.share({title:'개인연금 프로젝트 전체 백업',text:'Google Drive를 선택해 투자시스템/개인연금/프로젝트백업 폴더에 저장하세요.',files:[file]});state.meta.storage.driveStatus='공유 전달';state.meta.storage.driveVerification='미검증';state.meta.storage.lastDriveShare=v1LocalTime();await v1PersistNow({strict:true,syncCloud:false});v1ShowResult('공유 전달 완료 · Drive 확인 필요',`검증된 ${esc(pack.fileName)}을 공유창에 전달했습니다.<br>실제 Drive 저장 여부는 Drive 앱에서 확인해야 합니다.`)}else{v1Download(pack.blob,pack.fileName);v1ShowResult('프로젝트 ZIP 다운로드 완료',`직접 Drive 연결이나 안드로이드 공유가 없어 폰으로 내려받았습니다.<br>Google Drive의 투자시스템/개인연금/프로젝트백업 폴더에 직접 올려주세요.`)}}catch(e){state.meta.storage.driveStatus='실패';state.meta.storage.driveVerification='실패';v1ShowResult('프로젝트 백업 중단',esc(e?.message||e),true)}}
async function v1CloudSync(){const cloud=v1Adapters().cloud;if(!cloud?.load||!cloud?.save){v1ShowResult('클라우드 연결 전','Firebase 배포 설정이 연결되면 이 버튼에서 기기·클라우드 원본을 비교하고 동기화합니다.',true);return}try{v1ShowResult('클라우드 확인 중','원격 원본의 데이터 ID·수정 번호·수정 시각을 비교합니다.');await v1PersistNow({strict:true,syncCloud:false});const remote=await cloud.load();if(!remote){const result=await cloud.save(clone(state));state.meta.storage.cloudStatus='정상';state.meta.storage.lastCloudSave=v1LocalTime();state.meta.storage.cloudRevision=Number(result?.revision??state.meta.revision??0);await v1PersistNow({strict:true,syncCloud:false});return v1ShowResult('클라우드 원본 생성','현재 기기 데이터를 클라우드 원본으로 저장했습니다.')}const r=v1EnsureState(clone(remote));if(String(r.dataId)!==String(state.dataId)){state.meta.storage.cloudStatus='충돌';return v1ShowResult('클라우드 데이터 원본 충돌','클라우드와 기기의 dataId가 다릅니다. 자동 덮어쓰기를 중단했습니다.',true)}const localRev=Number(state.meta?.revision)||0,remoteRev=Number(r.meta?.revision)||0,localTime=Date.parse(state.meta?.updatedAt||0)||0,remoteTime=Date.parse(r.meta?.updatedAt||0)||0,localSig=v1CoreSignature(state),remoteSig=v1CoreSignature(r);if(remoteRev>localRev||(remoteRev===localRev&&remoteTime>localTime)){v1RestoreCandidate={data:r,manifest:{schemaVersion:r.schemaVersion,counts:v1Counts(r),createdAt:r.meta?.updatedAt,appVersion:r.meta?.appVersion},source:'cloud'};v1RenderRestorePreview()}else if(remoteRev===localRev&&remoteTime===localTime&&remoteSig!==localSig){state.meta.storage.cloudStatus='충돌';v1ShowResult('클라우드 동기화 충돌','수정 번호와 시간이 같지만 내용이 달라 자동 덮어쓰기를 중단했습니다.',true)}else{const result=await cloud.save(clone(state));state.meta.storage.cloudStatus='정상';state.meta.storage.lastCloudSave=v1LocalTime();state.meta.storage.cloudRevision=Number(result?.revision??localRev);await v1PersistNow({strict:true,syncCloud:false});v1ShowResult('클라우드 동기화 완료','기기 데이터가 클라우드 원본에 반영됐습니다.')}}catch(e){state.meta.storage.cloudStatus='실패';state.meta.storage.cloudError=String(e?.message||e);v1ShowResult('클라우드 동기화 실패',esc(e?.message||e),true)}}
...
function v1SanitizeImportedStorage(incoming,source='backup'){
  const previous=clone(incoming.meta?.storage||{});incoming.meta=incoming.meta||{};incoming.meta.importAudit={restoredAt:v1DateTime(),source,previousStorage:{lastLocalSave:previous.lastLocalSave||'',lastCloudSave:previous.lastCloudSave||'',lastDriveBackup:previous.lastDriveBackup||''}};
  incoming.meta.storage={localStatus:'대기',localVerification:'미검증',cloudStatus:'연결 전',driveStatus:'연결 전',driveVerification:'미검증',lastLocalSave:'',lastCloudSave:'',lastDriveBackup:'',lastDriveShare:'',lastBackup:'',lastRestore:'',lastValidation:''};return incoming;
}
...
async function v1ApplyRestore(){if(!v1RestoreCandidate?.migrated)return;const original=clone(state);try{v1ShowResult('복원 전 안전백업 생성 중','현재 데이터를 별도 보관한 뒤 복사본에서 변환합니다.');const safety=await v1BuildBackup(original),safetyWhere=await v1StoreSafetyBackup(original,safety);const incoming=v1SanitizeImportedStorage(clone(v1RestoreCandidate.migrated),v1RestoreCandidate.source||'backup'),valid=v1ValidateData(incoming);if(!valid.ok)throw new Error(valid.errors[0]);state=incoming;state.meta.storage.lastRestore=v1LocalTime();state.meta.storage.lastValidation='복원 정상';lastSignature=dataSignature(state);v1LastCore=v1CoreSignature(state);lastSignature=dataSignature(state);await v1PersistNow({strict:true,syncCloud:false});renderAll(true);v1RestoreCandidate=null;v1ShowResult('복원 완료',`교체 전 데이터는 ${esc(safetyWhere)}에 안전백업했습니다.<br>계좌 ${v1Counts(state).accounts}개 · 총자산 ${fmt(v1Counts(state).totalAssets)}`)}catch(e){state=original;lastSignature=dataSignature(state);v1LastCore=v1CoreSignature(state);renderAll(true);v1ShowResult('복원 실패 · 기존 데이터 유지',esc(e?.message||e),true)}}
...
window.PensionV1Data={
  ...
  persistNow:v1PersistNow,
  readLocal:()=>v1DBGet(V1_CURRENT),
  readSafety:()=>v1DBGet(V1_SAFETY),
  zipRead:v1ZipRead,
  zipCreate:v1ZipCreate,
  sha:v1Sha,
  filesFromState:v1FilesFromState
};
