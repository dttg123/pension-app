/* 개인연금 V3.1.1 입력·분류·시뮬레이션 완성 */
(()=>{
'use strict';
const V29_VERSION='3.1.1';
const V29_CLASSES=[
...
function v29OpenHoldingForm(prefKey=state.ui.accountView,editIndex=null){
  ...
  document.getElementById('v31DeleteHolding')?.addEventListener('click',()=>v31ConfirmDeleteHolding(prefKey,editIndex));openSheet('formSheet',true);
}
...
const v29PrevRenderAll=renderAll;renderAll=function(keep=false){v29EnsureState();v29PrevRenderAll(keep);v29EnhanceHome();document.title='개인연금 V3.1.1';v29UpgradeQuickSheet();const sub=document.getElementById('headerSub');if(sub)sub.textContent=`${v29CurrentAge()}세 · ${state.profile.retirementAge}세 연금 개시 계획`;state.meta.appVersion=V29_VERSION};
