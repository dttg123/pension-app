/* 개인연금 V2.5 최종 셸: 버전 고정, 진단, 저장 후처리, PWA 갱신 */
(()=>{
'use strict';
const VERSION='2.5.0';
...
function fixVersionUI(){
  ...
  document.querySelectorAll('.v0Badge,.v1Badge').forEach(b=>{b.textContent='V2.5';b.classList.add('v12Version')});
  const notice=document.querySelector('#settingsBody .sheetNotice');
  if(notice)notice.textContent='저장 버튼을 눌러야 반영됩니다. 앱 V2.5.0 · 데이터 구조 6 · 사진 원본 저장 안 함';
}
...
/* 하위 모듈이 과거 버전 문자열을 다시 쓰더라도 최종 저장값은 항상 2.5.0으로 고정한다. */
...
function addHistoryLedger(type,year,amount,{accountKey='',assetName=''}={}){
  ...
  state.ledger.push({
    ...
    source:HISTORY_SOURCE,synthetic:true,note:'V2.5 과거 연도 입력 원장 조정',createdAt:new Date().toISOString(),ledgerSchemaVersion:2
  });
}
